#ifndef CPPSQLITEDBADAPTER_TEST_UTILITIES_COMPARATORS_DBCOMPARATOR_H_
#define CPPSQLITEDBADAPTER_TEST_UTILITIES_COMPARATORS_DBCOMPARATOR_H_

#include "../Stubs/StubField.h"
...
inline testing::AssertionResult DbComparator(const db::IField &expected,
                                             const db::IField &actual) {
...
inline testing::AssertionResult DbComparator(const db::IFieldValue &expected,
                                             const db::IFieldValue &actual) {
  if (typeid(expected) != typeid(actual)) {
    return AssertionFailure() << "Different class type: "
                              << "expected=" << typeid(expected).name()
                              << ", actual=" << typeid(actual).name();
  ...
  if (!expected.isNull()) {
    ...
    if (fieldType == systelab::db::BOOLEAN &&
        (expected.getBooleanValue() != actual.getBooleanValue())) {
    ...
    if (fieldType == systelab::db::INT &&
        (expected.getIntValue() != actual.getIntValue())) {
    ...
    if (fieldType == systelab::db::DOUBLE &&
        (expected.getDoubleValue() != actual.getDoubleValue())) {
    ...
    if (fieldType == systelab::db::STRING &&
        (expected.getStringValue() != actual.getStringValue())) {
    ...
    if (fieldType == systelab::db::DATETIME &&
        (expected.getDateTimeValue() != actual.getDateTimeValue())) {
  ...
  AssertionResult stubFieldsComparisonResult =
      DbComparator(expectedStepField, actualStepField);
...
inline testing::AssertionResult DbComparator(const db::ITableRecord &expected,
                                             const db::ITableRecord &actual) {
  if (typeid(expected) != typeid(actual)) {
    return AssertionFailure() << "Different class type: "
                              << "expected=" << typeid(expected).name()
                              << ", actual=" << typeid(actual).name();
  ...
  for (unsigned int i = 0; i < nExpectedFieldValues; i++) {
    ...
    AssertionResult stubFieldsComparisonResult =
        DbComparator(fieldValue1, fieldValue2);
    if (!stubFieldsComparisonResult) {
      return AssertionFailure()
             << "Different data for field " << i << " "
...
#endif // CPP_SQLITE_DB_ADAPTER_TEST_UTILITIES_COMPARATORS_DBCOMPARATOR_H_
