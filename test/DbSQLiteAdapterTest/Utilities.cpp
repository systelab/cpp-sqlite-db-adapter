...
void createTable(IDatabase &db, std::string tableName,
                 unsigned int numRecords) {
  db.executeOperation(
      "CREATE TABLE " + tableName + " " + "(ID INT PRIMARY KEY, " +
      " FIELD_INT_INDEX INT, " + " FIELD_INT_NO_INDEX INT DEFAULT 2, " +
      " FIELD_STR_INDEX TEXT, " +
      " FIELD_STR_NO_INDEX TEXT DEFAULT 'FIELD_STR_NO_INDEX', " +
      " FIELD_REAL REAL DEFAULT 3.3, " + " FIELD_BOOL BOOLEAN DEFAULT 0, " +
      " FIELD_DATE DATETIME DEFAULT '20160102T030405') ");

  db.executeOperation("CREATE INDEX INT_INDEX ON " + tableName +
                      "(FIELD_INT_INDEX)");
  db.executeOperation("CREATE INDEX STR_INDEX ON " + tableName +
                      "(FIELD_STR_INDEX)");
...
void createPairOfTables(IDatabase &db, std::string tableName_t1,
  ...
  db.executeOperation(
      "CREATE TABLE " + tableName_t1 +
      "(ID INT PRIMARY KEY NOT NULL, FIELD_INT INT, FIELD_STR CHAR(255))");

  db.executeOperation("CREATE TABLE " + tableName_t2 +
                      "(ID INT, FIELD_INT_REF_T1 INT, FIELD_INT INT, FIELD_STR "
                      "CHAR(255), FOREIGN KEY(FIELD_INT_REF_T1) REFERENCES " +
                      tableName_t1 + "(ID) ON DELETE " + onDelete +
                      " ON UPDATE " + onUpdate + ")");
  ...
  for (unsigned int i = 0; i < numRecords_t1; i++) {
    ...
    oss << "INSERT INTO " + tableName_t1 +
               " (ID, FIELD_INT, FIELD_STR) VALUES ("
        << i << ", " << i % 4 << ", \"STR" << i % 9 << "\""
        << ")";
    db.executeOperation(oss.str());
  }
  ...
  for (unsigned int i = 0; i < numRecords_t2; i++) {
    ...
    oss << "INSERT INTO " + tableName_t2 +
               " (ID, FIELD_INT_REF_T1, FIELD_INT, FIELD_STR) VALUES ("
        << i << ", " << i % 4 << ", " << i % 20 << ", \"STR" << i % 9 << "\""
        << ")";
    db.executeOperation(oss.str());
  }
...
int getFieldIntIndexValue(unsigned int id) { return id % 7; }

int getFieldIntNoIndexValue(unsigned int id) { return id % 10; }
...
double getFieldRealValue(unsigned int id) { return (id % 13) / 10.; }

bool getFieldBooleanValue(unsigned int id) { return (id % 2 == 0); }

boost::posix_time::ptime getFieldDateValue(unsigned int id) {
  return boost::posix_time::ptime(getFieldDateBaseDate() +
                                  boost::gregorian::days(id % 7));
}
...
unsigned int
getNumRecordsWithFieldStringNoIndexZero(unsigned int tableRecords) {
