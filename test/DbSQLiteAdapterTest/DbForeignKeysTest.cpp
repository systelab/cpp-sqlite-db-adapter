...
//  IDatabase &db = *(m_db.get());
//  createPairOfTables(db, TABLE_1, 100, TABLE_FOREING_KEYS, 50, "NO ACTION",
//                     "NO ACTION");
...
//      "SELECT * FROM " + TABLE_FOREING_KEYS + " WHERE FIELD_INT_REF_T1 = 1");
...
//  createPairOfTables(db, TABLE_1, 100, TABLE_FOREING_KEYS, 50, "CASCADE",
//                     "NO ACTION");
...
//  IDatabase &db = *(m_db.get());
//  createPairOfTables(db, TABLE_1, 100, TABLE_FOREING_KEYS, 50, "RESTRICT",
//                     "NO ACTION");
...
//  IDatabase &db = *(m_db.get());
//  createPairOfTables(db, TABLE_1, 100, TABLE_FOREING_KEYS, 50, "SET NULL",
//                     "NO ACTION");

//  m_db->executeOperation("DELETE FROM " + TABLE_1 + " WHERE ID = 1");

//  std::unique_ptr<IRecordSet> recordSet =
//      m_db->executeQuery("SELECT * FROM " + TABLE_FOREING_KEYS +
//                         " WHERE FIELD_INT_REF_T1 IS NULL");
...
//  ASSERT_THROW(m_db->executeOperation("UPDATE " + TABLE_1 +
//                                      " SET ID = 1800 WHERE ID = 1"),
//               std::exception);
...
//  std::unique_ptr<IRecordSet> recordSet = m_db->executeQuery(
//      "SELECT * FROM " + TABLE_FOREING_KEYS + " WHERE FIELD_INT_REF_T1 =
//      1800");

//  ASSERT_NE(recordSet->getRecordsCount(), 0);

//  recordSet = m_db->executeQuery("SELECT * FROM " + TABLE_FOREING_KEYS +
//                                 " WHERE FIELD_INT_REF_T1 = 1");
//  ASSERT_EQ(recordSet->getRecordsCount(), 0);
//}
